<template>
  <div class="w-full h-screen px-10 py-5 bg-gray-100">
    <div class="flex mb-20">
      ...
      <p class="text-3xl font-bold">Update Pegawai</p>
    </div>
    <form class="max-w-sm mx-auto" @submit.prevent="update">
      ...
      <button
        type="submit"
        class="text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg w-full sm:w-auto px-5 py-2.5 text-center mb-5"
      >
        Submit
      </button>
      <div
...
export default {
  setup() {
    ...
    function getEmployee() {
      axios
        .get(`http://localhost:3000/api/employees/${route.params.id}`)
        .then((response) => {
          employee.employee_name = response.data.data.employee_name
          employee.email = response.data.data.email
          employee.position_id = response.data.data.position_id
        })
        .catch((error) => {
    ...
    function update() {
      ...
      axios
        .patch(`http://localhost:3000/api/employees/update/${route.params.id}`, {
          employee_name,
          email,
          position_id,
        })
